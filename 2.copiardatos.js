...
async function run() {
    ...
    const takeSnap = async (name) => {
        ...
        try {
            ...
            screenshots[`paso_${Object.keys(screenshots).length + 1}_${name}`] = {
    ...
    try {
        ...
        //    Buscar TODAS las filas con tipo ANULACION
        ...
        try {
            ...
            const resultados = await popupPage.evaluate(() => {
                ...
                const TIPOS_ANULACION = [
                    "ANULACION TOTAL",
                    "ANULACION POLIZAS POR FALTA DE PAGO",
                    "ANULACION MASIVA"
                ];
                ...
                for (let i = 0; i < tabla.rows.length; i++) {
                    ...
                    if (TIPOS_ANULACION.includes(celdaDesc.innerText.trim())) {
                        coincidencias.push({
            ...
            if (resultados) {
                ...
                console.warn('⚠️ No se encontró ninguna fila con tipo ANULACION');
                await takeSnap('tabla_sin_anulacion');
            }
